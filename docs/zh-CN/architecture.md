...
App 页面只等待核心应用目录即可进入可交互状态，主屏幕位置和 WDA 状态会独立在后台补全。目录按设备范围短期缓存；离开页面或切换设备会取消请求，设备事件及应用变更会使缓存失效。前端按固定批次创建列表行，图标观察器只拉取接近可视区域的资源，避免为大型目录一次创建完整的 Ant Design 与图标子树。
