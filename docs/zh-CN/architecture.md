...
前端的 `useDeviceVideoStream` controller 独占视频 WebSocket、WebCodecs 解码、Canvas 呈现、重连、画面需求、停滞检测和前端性能指标。`App` 只组合页面工作流，通过稳定的命令接口发送输入，并在断线回调中清理按键映射状态。`useDeviceMediaCapture` 独立拥有截图对象 URL、原生截图请求互斥、`MediaRecorder`、捕获轨道和卸载清理；它只接收视频控制器的 Canvas 能力与当前设备上下文，因此捕获工作流既不能参与解码器生命周期，也不能在工作区结束后继续保留资源。
