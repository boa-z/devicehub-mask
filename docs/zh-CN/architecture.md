...
但私有 API 只开放有界目录列表和普通文件导出。路径会拒绝目录穿越、反斜杠、NUL 与不安全
组件，符号链接和特殊条目不会被继续访问。导出使用 64 KiB 缓冲流，对照初始 AFC 元数据
校验字节数，并通过目标同目录的临时文件原子替换原生对话框所选目标。上传、重命名、删除、
创建目录、递归导出和 MCP 工具均不在此边界内。
