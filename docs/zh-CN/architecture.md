...
更新时间。虚拟定位、Sysmontap、Graphics 与 NetworkMonitor 通道分别使用有上限的
指数退避恢复；单个通道断开不会终止视频或 HID。

性能监控复用活动软件隧道的克隆 handle，并建立相互隔离的 DVT 连接。性能工作台或所选 HUD
指标有需求时才启动 Sysmontap、Graphics 和 NetworkMonitor 采样。NetworkMonitor 使用独立
RemoteServer 连接，按连接累计计数器的差值计算每秒收发速率；超过一分钟未更新的连接
