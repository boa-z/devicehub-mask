...
App 页面只等待核心应用目录即可进入可交互状态，主屏幕位置和 WDA 状态会独立在后台补全。目录按设备范围短期缓存；离开页面或切换设备会取消请求，设备事件及应用变更会使缓存失效。前端按固定批次创建列表行，图标观察器只拉取接近可视区域的资源，避免为大型目录一次创建完整的 Ant Design 与图标子树。每个应用行由 Apps 功能模块拥有，只接收稳定命令而不接触父级状态 setter；组件记忆化会隔离备份、诊断、壁纸及其他检查器状态更新。备份、sysdiagnose、开发者镜像和 App 操作状态统一使用“仅活动任务轮询”策略：进入所属 Tab 时读取一次快照，只在操作处于活动状态时轮询，并在终态立即停止；请求代际会阻止旧设备或较早轮询的延迟响应覆盖当前状态。
