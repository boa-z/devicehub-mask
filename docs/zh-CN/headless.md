...
可以同时保持多台设备连接。每个浏览器标签页选择一个包含传输信息的准确设备 ID，其 HTTP 与 WebSocket 请求始终限定到该会话。只有页面正在显示对应设备时才执行视频处理；只有至少一个页面为该设备请求已启用且未静音的音频时才启动音频解码，全部查看者静音后会释放解码器。性能采样与设备日志也分别按设备、按需求启停。关闭标签页会自动释放其媒体需求，轻量设备会话则继续保留以便快速切换。
