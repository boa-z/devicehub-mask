...
浏览器界面通过经过鉴权的 headless API 读取构建信息、能力声明、设置和诊断状态。headless 设置保存在 `<data-dir>/settings.json`，浏览器错误会转发到服务进程日志。浏览器全屏可用。设备音频由内置 FFmpeg 解码，经同一鉴权 WebSocket 以有界的 48 kHz 双声道 PCM 发送给 Web Audio；受浏览器自动播放策略影响，开始播放前可能需要点击一次页面。窗口置顶、安装器更新、原生文件选择框、打开服务端目录和宿主剪贴板同步等桌面专属能力会被明确禁用，不再错误调用 Tauri 命令。浏览器原生文件传输是后续对齐阶段。
