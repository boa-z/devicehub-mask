...
The browser UI reads build information, capabilities, settings, and diagnostics from the authenticated headless API. Headless preferences are stored in `<data-dir>/settings.json`, and browser errors are forwarded to the service log. Browser fullscreen is supported. Device audio uses the bundled FFmpeg decoder and sends bounded 48 kHz stereo PCM over the authenticated WebSocket to Web Audio; browser autoplay policy may require one click before playback starts. AFC and application storage support browser-native single-file upload up to 64 MiB and download up to 256 MiB, and crash reports can be downloaded directly from either browser report view. Transfers use isolated random staging under `<data-dir>/transfers` and are removed after each operation; directory transfer remains desktop-only. Desktop-only capabilities such as always-on-top windows, installer updates, native file dialogs, opening server directories, and host clipboard synchronization are disabled instead of invoking unavailable Tauri commands.
