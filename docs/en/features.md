...
- Key mapping: `list_keymap_profiles`, `get_keymap_profile`, `save_keymap_profile`, `run_keymap`, `start_game_session`, `set_game_input`, and `stop_game_session`. Agents can create native v2 profiles and run persistent 60Hz mapping playback on their selected device; the renewable lease releases controls automatically when the Agent stops updating.
