...
Playback supports `touch`, `dpad`, `Press`, `SingleTap`, `RepeatTap`, `MultipleTap`, `Swipe`, keyboard-button `DirectionPad`, `PadCastSpell`, `MouseCastSpell`, `CancelCast`, `Observation`, `Fps`, `Fire`, and `hardwareBindings`. A `Press` contact stays down for the complete `run_keymap.hold_ms` interval or while its key remains held in a game session. `DirectionPad`, cast mappings, and pointer-driven mappings use `targetResolution` when present, otherwise the selected device's current upright stream size.

In a game session, `pointer_deltas` move the named active `MouseCastSpell`, `Observation`, `Fps`, or non-preserving `Fire` mapping. A pointer delta may also include normalized `cursor_x` and `cursor_y`; MouseCastSpell uses that absolute cursor position with `center`, `cast_radius`, `drag_radius`, and its axis scale factors, while omitting both fields preserves relative-delta compatibility. `Observation` clamps to `max_radius`. `Fps` is a press-edge toggle: include its binding in one `set_game_input` update and omit it from the next; it remains active and accepts deltas until another press edge disables it. Its single/dual recenter strategy runs locally at 60Hz. `Fire` either coexists with FPS as a stationary contact or temporarily takes over pointer control according to `preserve_fps_control`. `CancelCast` animates the current cast to its cancel point before release. Randomized anchors, swipe curves, direction-pad distance scaling, and bounded drift are evaluated by the shared native runtime; the browser only sends key, axis, and pointer state.

`Script` mappings and script hooks use the same bounded runtime as desktop control, but MCP keeps them disabled unless `allow_scripts=true` is explicitly supplied to `run_keymap` or `start_game_session`. Inspect a profile before opting in. Scripts can emit touches, keyboard usages, hardware buttons, Unicode text, and shared FPS/cast actions. MCP rejects `enter_raw_input()` because changing an Agent connection into desktop keyboard passthrough has no defined protocol meaning. A standalone `RawInput` mapping also remains an explicit error.

## Device and Session Workflow

Use `list_devices` for the current transport inventory and pass its exact transport-aware `id` to `connect_device` or `reconnect_device`; bare UDIDs are not accepted. `status` reports this MCP connection's selected session, while `list_operations` returns that session's bounded long-operation lifecycle and typed errors. Pass a cancellable operation ID to `cancel_operation`; cancellation stays scoped to the selected device and the runtime routes it to the owning service. Connecting selects or reuses the exact session without stopping sessions for other physical devices. Reconnecting tears down and rebuilds only that target. Both wait for a new video frame for a bounded period and may report that connection is still being established; follow with `status` or `screenshot` rather than repeatedly reconnecting.
...
| Device and session | `status`, `device_details`, `list_devices`, `list_operations`, `cancel_operation`, `connect_device`, `reconnect_device`, `wait_for_device_event`, `list_companion_devices`, `home_screen_layout` | Exact selection IDs preserve USB/Wi-Fi identity; stable identifiers are opt-in |
