...
Device-scoped private HTTP calls carry `X-DeviceHub-Device`, and WebSocket clients use `device_id` in the query string. Missing or unknown targets are rejected rather than falling back to whichever device the desktop currently displays. The connection-center projection groups transports by physical UDID but preserves transport-aware selection and per-row errors. Each MCP protocol connection owns an independent selected target, so two agents may operate different connected devices without changing the desktop selection.

Connected sessions retain only the transport state required for quick switching. Video and audio use independent, composable consumer leases per session. With no video consumer, RTP/RTCP remains drained and observable, but the runtime skips HEVC depacketization, access-unit assembly, and publication; the first resumed consumer clears stale state and requests a keyframe. Desktop audio starts FFmpeg only for the selected device. Headless audio starts FFmpeg only while at least one browser client requests unmuted playback for that exact selection ID. Performance sampling and device-log streaming remain separately demand-gated. The status projection exposes these four resource states per session so idle multi-device cost can be diagnosed without enabling the resources being inspected.
