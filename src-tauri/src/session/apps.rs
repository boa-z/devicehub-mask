...
use crate::protocol::{AppOperationKind, AppOperationSlot, DeviceApp, InputCmd};
use idevice::{
...
pub(super) struct AppServiceTransport {
    adapter: AdapterHandle,
    handshake: RsdHandshake,
}

impl AppServiceTransport {
    pub(super) fn new(adapter: AdapterHandle, handshake: RsdHandshake) -> Self {
        Self { adapter, handshake }
    }
}

pub(super) struct AppClientSet {
    app_service: Option<AppServiceClient<Box<dyn ReadWrite>>>,
    installation_proxy: Option<InstallationProxyClient>,
}

impl AppClientSet {
    pub(super) async fn connect_installation_proxy(provider: &dyn IdeviceProvider) -> Self {
    ...
    pub(super) async fn connect_app_service(
        &mut self,
...
pub(super) struct AppManagement {
...
impl AppManagement {
    ...
    pub(super) async fn handle(&mut self, command: InputCmd) -> Option<InputCmd> {
        match command {
            InputCmd::ListApps {
                include_system,
                include_app_clips,
...
mod tests {
    use super::*;
    use crate::protocol::AppOperationState;
