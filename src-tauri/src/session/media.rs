...
impl RunningStats {
    ...
    pub(super) fn standard_deviation(&self) -> Option<f64> {
        (self.count > 0).then(|| (self.squared_deviations / self.count as f64).sqrt())
    }
}

#[derive(Debug)]
pub(super) struct HevcAccessUnit {
    pub(super) bytes: Vec<u8>,
    pub(super) is_irap: bool,
    pub(super) rtp_timestamp: u32,
}

#[derive(Debug, Default)]
pub(super) struct AccessUnitAssembler {
    pending: Vec<u8>,
    pending_timestamp: Option<u32>,
}

impl AccessUnitAssembler {
    pub(super) fn push(&mut self, bytes: &[u8], rtp_timestamp: u32) -> Vec<HevcAccessUnit> {
    ...
    pub(super) fn finish(&mut self) -> Option<HevcAccessUnit> {
        if self.pending.is_empty() {
            return None;
        }
...
#[derive(Debug, Default)]
pub(super) struct RtpVideoClock {
    last_timestamp: Option<u32>,
    elapsed_ticks: u64,
}

impl RtpVideoClock {
    pub(super) fn timestamp_us(&mut self, timestamp: u32) -> u64 {
...
impl HevcQueue {
    ...
    pub(super) fn push(&self, access_unit: HevcAccessUnit) -> HevcQueuePush {
    ...
    pub(super) fn force_resync(&self) -> (u64, u64) {
        let mut state = self.state.lock().unwrap();
        state.dropped_access_units = state
            .dropped_access_units
            .saturating_add(state.access_units.len() as u64);
    ...
    pub(super) async fn pop(&self) -> Option<HevcAccessUnit> {
        loop {
    ...
    pub(super) fn take_snapshot(&self) -> HevcQueueSnapshot {
        let mut state = self.state.lock().unwrap();
...
#[derive(Debug, Clone, Copy)]
pub(super) struct HevcQueueSnapshot {
    pub(super) queued_access_units: usize,
    pub(super) queued_bytes: usize,
    pub(super) peak_bytes: usize,
    pub(super) waiting_for_irap: bool,
    pub(super) wait_ms: f64,
    pub(super) wait_max_ms: f64,
}

pub(super) fn audio_decoder_restart_backoff(attempt: u32) -> Duration {
