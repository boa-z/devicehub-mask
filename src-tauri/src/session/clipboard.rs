...
use crate::protocol::{ClipboardContentKind, ClipboardEvent, ClipboardSlot, clipboard_preview};

/// How often to poll the host clipboard for host -> device changes. Arboard has
/// no portable change notification; device -> host remains push-driven.
...
pub(super) struct ClipboardCommands(Receiver<ClipboardCommand>);

#[derive(Clone)]
pub(super) struct ClipboardBridge(Sender<ClipboardCommand>);

impl ClipboardBridge {
    pub(super) fn channel() -> (Self, ClipboardCommands) {
        let (sender, receiver) = tokio::sync::mpsc::channel(CLIPBOARD_COMMAND_CAPACITY);
        (Self(sender), ClipboardCommands(receiver))
    }

    pub(super) async fn set_text(&self, text: String) -> Result<(), String> {
...
/// Own the single pasteboard connection for a session. A second connection can
/// displace the existing device subscriber, so explicit paste commands and
/// automatic synchronization deliberately share this task.
pub(super) async fn run(
    ...
    let ClipboardCommands(mut commands) = commands;
    ...
    let mut host = match arboard::Clipboard::new() {
        Ok(clipboard) => clipboard,
        Err(error) => {
            tracing::warn!("no host clipboard; clipboard sync disabled: {error:?}");
            command_loop(
                Some(pasteboard),
                &activity,
                &mut commands,
    ...
    loop {
        // Dropping recv_push when another branch wins is safe because the XPC
        // read path buffers partial reads.
        ...
        match wake {
            ClipboardWake::Push(Ok(snapshot)) => {
                apply_device_snapshot(&snapshot, &mut host, &activity, &mut state)
            }
            ClipboardWake::Push(Err(error)) => {
                tracing::warn!("clipboard PUSH failed: {error:?}");
            ...
            ClipboardWake::Tick => {
                match pasteboard.get(GENERAL_PASTEBOARD).await {
                    ...
                    Err(error) => {
                        tracing::warn!("clipboard PULL failed: {error:?}");
                        if let Some(client) = reconnect(adapter, handshake).await {
                            pasteboard = client;
                            subscribe(&mut pasteboard).await;
                        }
                ...
                if let Err(error) =
                    push_host_clipboard(&mut pasteboard, &mut host, &activity, &mut state).await
                {
                    tracing::warn!("clipboard host -> device failed: {error:?}");
                    if let Some(client) = reconnect(adapter, handshake).await {
                        pasteboard = client;
                        subscribe(&mut pasteboard).await;
                    }
...
async fn subscribe(pasteboard: &mut PasteboardServiceClient<Box<dyn ReadWrite>>) {
    if let Err(error) = pasteboard
        ...
        .await
    {
        tracing::warn!("clipboard: failed to subscribe to change notifications: {error:?}");
    }
}

fn apply_device_snapshot(
    snapshot: &PasteboardSnapshot,
    host: &mut arboard::Clipboard,
    activity: &ClipboardSlot,
    state: &mut ClipState,
) {
    ...
    if let Some(text) = snapshot.text() {
        if Some(&text) != state.last_text.as_ref() {
            match host.set_text(text.clone()) {
                ...
                Err(error) => tracing::warn!("failed to set host text: {error:?}"),
            }
        }
            ...
            Some(image) => {
                ...
                if Some(hash) != state.last_image {
                    match host.set_image(image) {
                        Ok(()) => {
                            tracing::info!("clipboard: device -> host (image {width}x{height})");
                            activity.set(ClipboardEvent {
                                from_device: true,
                        ...
                        Err(error) => tracing::warn!("failed to set host image: {error:?}"),
                    }
                }
            }
            ...
            match encode_png(&image) {
                Some(png) => {
                    pasteboard
                        ...
                        .await?;
                    tracing::info!(
                        "clipboard: host -> device (image {width}x{height}, {} bytes png)",
                        png.len()
                    );
                    activity.set(ClipboardEvent {
                        from_device: false,
...
async fn reconnect(
    ...
    .await
    {
        ...
        Ok(Err(error)) => {
            tracing::warn!("clipboard reconnect failed: {error:?}");
            None
        }
        Err(_) => {
...
mod tests {
    ...
    use super::{decode_image, encode_png, image_hash};
