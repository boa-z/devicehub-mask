...
pub(crate) enum FrameCredit {
    BrowserAccepted(u64),
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum BrowserFrameDecision {
    Send { completes_resync: bool },
    SkipForResync,
    Backpressured { entered_resync: bool },
}

pub(crate) fn browser_frame_decision(
    key: bool,
    sequence: u64,
    resync: &AtomicBool,
    pacer: &FramePacer,
...
pub(crate) fn configured_in_flight_frames(value: Option<&std::ffi::OsStr>) -> usize {
...
pub(crate) fn duration_average_ms(total: Duration, samples: u64) -> f64 {
    if samples == 0 {
        0.0
    } else {
...
#[derive(Debug, Clone, Copy)]
pub(crate) struct FramePacerMetrics {
    pub(crate) decoder_accept_average_ms: f64,
    pub(crate) decoder_accept_max_ms: f64,
    pub(crate) presentation_average_ms: f64,
    pub(crate) presentation_max_ms: f64,
    pub(crate) expired_credits: u64,
}
...
impl FramePacer {
    ...
    pub(crate) fn try_acquire(&self, kind: FrameCredit) -> bool {
    ...
    pub(crate) fn release(&self, kind: FrameCredit) {
        let mut state = self.state.lock().expect("frame pacer lock poisoned");
    ...
    pub(crate) fn presented(&self, sequence: u64) {
        let mut state = self.state.lock().expect("frame pacer lock poisoned");
    ...
    pub(crate) fn clear_browser(&self) {
        let mut state = self.state.lock().expect("frame pacer lock poisoned");
        state.pending.clear();
    ...
    pub(crate) fn take_metrics(&self) -> FramePacerMetrics {
        let mut state = self.state.lock().expect("frame pacer lock poisoned");
