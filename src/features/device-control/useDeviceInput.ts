...
import type { KeymapStatus } from "../device-media/useDeviceRealtimeSession";
...
type Options = {
  connected: boolean;
  sendControl: (payload: DeviceInputCommand) => void;
  profile: Profile;
...
export function useDeviceInput(options: Options) {
  ...
  const sendDirectTouches = useCallback(() => {
    ...
    current.sendControl(directTouchCommand(current.controlMode, [...directTouchesRef.current.values()]));
  }, []);
  ...
  const releaseAllControls = useCallback(() => {
    ...
    exitPointerLock();
    if (!current.connected) return;
    for (const usage of forwarded) current.sendControl({ type: "keyboard_up", usage });
    if (current.controlMode === "mapping") {
      current.sendControl({ type: "keymap_input", keys: [], pointer_deltas: [], gamepad_axes: {} });
      current.sendControl({ type: "keymap_direct_touches", contacts: [] });
    } else {
      current.sendControl({ type: "multi_touch", contacts: [] });
    }
  ...
  useEffect(() => {
    ...
    if (controlMode === "mapping") {
      ...
      sendControl({ type: "keymap_stop" });
    }
  }, [connected, controlMode, frameSize, profile, sendControl, sendDirectTouches, sendKeyState]);

  useEffect(() => {
    if (!connected) return;
    sendControl({ type: "keymap_debug", enabled: pointerDebugEnabled });
  }, [connected, pointerDebugEnabled, sendControl]);
  ...
  useEffect(() => {
    const down = (event: KeyboardEvent) => {
      ...
      if (current.controlMode === "keyboard") {
        ...
        current.sendControl({ type: "keyboard_down", usage });
        return;
      }
    ...
    const up = (event: KeyboardEvent) => {
      ...
      if (usage !== undefined) {
        ...
        current.sendControl({ type: "keyboard_up", usage });
        return;
      }
