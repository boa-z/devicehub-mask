...
import { Alert, Breadcrumb, Button, Empty, Input, Modal, Spin, Tooltip, Typography, message } from "antd";
...
function endpoint(bundleId: string, suffix = "") {
  return `/api/device/apps/${encodeURIComponent(bundleId)}/documents${suffix}`;
}
...
export function AppDocumentsModal({ app, request, onClose }: Props) {
  ...
  return (
    <Modal
      ...
      open={app !== null}
      width={760}
      title={app ? t("deviceInspector.appDocumentsTitle", { name: app.name }) : ""}
      footer={null}
      destroyOnHidden
      closable={busy === null}
      keyboard={busy === null}
