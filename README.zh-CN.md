...
- 设备信息、带真实图标的应用浏览与启动、IPA 安装、安全卸载、App Documents 文件管理、
  确认式设备重启/关机和描述文件检查
- 通过 CoreDevice Pasteboard Service 可选启用文本与图片的双向剪贴板同步
- 按需采集归一化 iPhone CPU、高负载进程 CPU/内存排行、Core Animation FPS 和 GPU
  内存，并监督 DVT 服务恢复
